...
public class TabLoginDAO extends HibernateDaoSupport {
	...
	public TabLogin findById(com.converter.TabLoginId id) {
		...
		try {
			TabLogin instance = (TabLogin) getHibernateTemplate().get(
					"com.converter.TabLogin", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding TabLogin instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from TabLogin as model where model."
					+ propertyName + "= ?";
	...
	public TabLogin merge(TabLogin detachedInstance) {
		...
		try {
			TabLogin result = (TabLogin) getHibernateTemplate().merge(
					detachedInstance);
