 ...
 * 
 * @see com.converter.SvgFile
 * @author MyEclipse Persistence Tools
 */

public class SvgFileDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory.getLogger(SvgFileDAO.class);
	// property constants
	...
	public SvgFile findById(com.converter.SvgFileId id) {
		log.debug("getting SvgFile instance with id: " + id);
		try {
			SvgFile instance = (SvgFile) getHibernateTemplate().get(
					"com.converter.SvgFile", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding SvgFile instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from SvgFile as model where model."
					+ propertyName + "= ?";
	...
	public List findBySourcefile(Object sourcefile) {
		return findByProperty(SOURCEFILE, sourcefile);
	}
	...
	public SvgFile merge(SvgFile detachedInstance) {
		...
		try {
			SvgFile result = (SvgFile) getHibernateTemplate().merge(
					detachedInstance);
